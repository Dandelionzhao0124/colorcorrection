function densematch(image1, image2, imgname)%输入图像1和图像2的地址，以及图像文件名 
str1='H:\intrinsic_image_work_record\data_IN\Dolls\';
%mkdir(str1,imgname);
...
imga = single(rgb2gray(imga));
imgb = single(rgb2gray(imgb));

binSize = 8;
magnif = 3;
imgas = vl_imsmooth(imga, sqrt((binSize/magnif)^2 - .25));
imgbs = vl_imsmooth(imgb, sqrt((binSize/magnif)^2 - .25));

[fa, da] = vl_dsift(imgas, 'size', binSize);
[fb, db] = vl_dsift(imgbs, 'size', binSize);
...
k = 1;
for i = 1:matchnum
    if(abs(ya(1,i)-yb(1,i))<20 && abs(xa(1,i)-xb(1,i))<400)
        img1x(1,k) = xa(1,i);
        img1y(1,k) = ya(1,i);
        img2x(1,k) = xb(1,i);
        img2y(1,k) = yb(1,i);
