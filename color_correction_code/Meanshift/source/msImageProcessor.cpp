...
{   //定义图像数据
	...
	DefineLInput(luv, height_, width_, dim); //设置lattice格形数据
	...
	if(!h)
	{
		...
		DefineKernel(k, tempH, P, 2); //定义核函数，算法默认使用单位均匀核函数flat kernel function
	}
...
{   //处理 SpeedUplevel为速度等级，值为None,Medium,high,速度越快，精度越小
	//根据speedUpLevel分别调用NewNonOptimizedFilter(),**Filter1(),**Filter2()进行meanshift迭代计算
	...
	case MED_SPEEDUP:
		...
		NewOptimizedFilter1((float)(sigmaS), sigmaR);//迭代计算
		break;
		//high speedup
	case HIGH_SPEEDUP:
...
{   //1、获取粗分割后的图像，filtImage必须预先分配内存
	//2、获取合并区域后的最终结果，segmImage必须预先分配内存
...
int msImageProcessor::GetRegions(int **labels_out, float **modes_out, int **MPC_out)
{
	...
	int		*labels_	= *labels_out;
	int     *MPC_out_   = *MPC_out;
...
void msImageProcessor::Connect( void )
{
	...
	//define eight connected neighbors 定义8个邻接关系
	neigh[0]	= 1;
	neigh[1]	= 1-width;
	neigh[2]	= -width;
	neigh[3]	= -(1+width);
