...
public: 
	...
	Mat reflect_sim_corr, shading_sim_corr, init_sim_corr, reflect_weight_corr, shading_weight_corr, init_weight_corr;
	Mat srcrLabIm, refrLabIm, srcsLabIm, refsLabIm;       // Lab空间下的图像
	Mat final_correctedSrcIm, final_wcorrectedSrcIm;    //若初始图像为1，则需要进行转换
	
public:
	void initial(const string folder, const RWTransfer& rwtranfer, const vector<vector<bool>>& ralist);
	void calGlobalParameters(Mat srcIm, Mat refIm);
	void calLocalParameters(Mat srcLabIm, Mat refLabIm);

	//global color transfer algorithims
	//将srcIm，中有效的区域（msk）使用srcMean, refMean, rsFactor进行颜色校正
	void correct(const Mat& msk, const Scalar& srcMean, const Scalar& refMean, const Scalar& rsFactor, Mat& sLabIm, Mat srcLabIm);

	void simpleCorrection(Mat srcIm);
	void weightedCorrection(Mat srcIm);
	...
	void Correction(Mat srcLabIm, Mat refLabIm, string sceneFn, string folder, string srcFn);
	...
	vector<double> getIMmap(int y, int x, Mat srcIm);
};
