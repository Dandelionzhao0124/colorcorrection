...
void calmatchpoints(vector<CMatch> matches, Mat srcSegmentsIm) {
	int sz = matches.size();
	Mat tim = srcSegmentsIm.clone();
	for(int i = 0; i < sz; ++i) {
		CMatch m = matches[i];
		Point2f p = m.srcPt;
		cv::circle(tim, p, 1, Scalar(255, 255, 0), 2);
	}
	imwrite("H:/intrinsic_image_work_record/data_0506_I2/Art2/pointsshow.png", tim);
}
