...
void PixelsVector2Mat(const std::vector<PixelType>& inpixels, int width, int height, int channel, cv::Mat& outmtx)
{
	...
	for (j = 0; j < height; j ++)
	{
		for (i = 0; i < width; i ++)
		{
			...
			if (channel == 3)
				outmtx.at<cv::Vec3b>(j, i) = cv::Vec3i( static_cast<uchar>(static_cast<int>(inpixels[index])),
				static_cast<uchar>(static_cast<int>(inpixels[index + 1])),
				static_cast<uchar>(static_cast<int>(inpixels[index + 2])) );
			else if (channel == 1)
				outmtx.at<uchar>(j, i) = static_cast<uchar>(static_cast<int>(inpixels[index]));				
		}
	}	
}
