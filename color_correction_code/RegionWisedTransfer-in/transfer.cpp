...
void RWTransfer::initial(const string& folder, const string& sceneFn, const string& refFn, const string& srcFn)
{
	...
	Mat srcgtmp, refgtmp;
	cvtColor(refRAWIm, refgtmp, CV_BGR2GRAY);
	cvtColor(srcRAWIm, srcgtmp, CV_BGR2GRAY);
	cvtColor(refgtmp, refGrayIm, CV_GRAY2BGR);
	cvtColor(srcgtmp, srcGrayIm, CV_GRAY2BGR);
	...
	//srcImFn = folder + "/"+ srcFn + "_con_7_4_300.png";
	...
	//string srcLabelFn = folder + "/" + srcFn + "_labels_7_4_300.txt";
	...
	//读取匹配点
	//string matchFn = folder + "/refine_matches.txt";
	...
	//为ref2src设置匹配标签
	calmatchpoints(matches, srcSegmentsIm);

	labelmap.resize(refregionum, -1);

	int sz = matches.size();
	cout << "sz" << sz << endl;
	vector<Point2f> refpointshows(sz);
	for(int i = 0; i < sz; ++i) {
		refpointshows[i].x = matches[i].refPt.x;
		refpointshows[i].y = matches[i].refPt.y;
	}
	Mat tim = refSegmentsIm.clone();
	for(int i = 0; i < sz; ++i)
	{
		cv::circle(tim, refpointshows[i], 2, Scalar(255, 255, 0), 2);
	}
	imwrite("H:/intrinsic_image_work_record/data_0506_I2/Art0/refpointshow.png", tim);
...
{
	fstream fin(ImFn.c_str(), ios::in);
	if(fin.is_open() == false) {
		cout << "failed to open" <<ImFn << endl;
		return;
	}
	if(channels == 3) {
		...
		for(int i = 0; i < rows; ++i) {
			for(int j = 0; j < cols; ++j) {
				fin >> RAWIm.at<Vec3f>(i, j)[0] >> RAWIm.at<Vec3f>(i, j)[1] >> RAWIm.at<Vec3f>(i, j)[2];
			}
	    }
	}
	else if(channels == 1) {
...
void RWTransfer::calRColorsin(vector<Scalar>& regioncolor, int regionum, vector<vector<Point2f>> Pixels, Mat Im)
{
	regioncolor.resize(regionum, 0);
	//每个区域的平均颜色和
	Scalar rcolorsum;
	...
	for(int i = 0; i < regionum; ++i)
	{
		...
		for(int j = 0; j < rnum; ++j)
		{
			int x = Pixels[i][j].x;
			int y = Pixels[i][j].y;
			rcolorsum[0] += Im.at<Vec3f>(y, x)[0];
			rcolorsum[1] += Im.at<Vec3f>(y, x)[1];
			rcolorsum[2] += Im.at<Vec3f>(y, x)[2];
		}
		regioncolor[i][0] = rcolorsum[0] / rnum;
		regioncolor[i][1] = rcolorsum[1] / rnum;
		regioncolor[i][2] = rcolorsum[2] / rnum;
...
void RWTransfer::showSameColor(string sfn, string flag)
{
	...
	float alpha = 0.5;
	float beta = 0.5;
	addWeighted(refGrayIm, alpha, markref, beta, 0.0, flagref);
	addWeighted(srcGrayIm, alpha, marksrc, beta, 0.0, flagsrc);

	if(sfn1 != "")
	{
		imwrite(sfn1, flagref);
		cout << "save " << sfn1 <<endl;
	}
	if(sfn2 != "")
	{
		imwrite(sfn2, flagsrc);
		cout << "save " << sfn2 <<endl;
	}
